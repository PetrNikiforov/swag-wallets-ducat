...
%% basic handler
-module(swag_server_wallet_terminals_handler).
...
allowed_methods(
    Req,
    State = #state{
        operation_id = 'GetTerminalAccount'
    }
) ->
    {[<<"GET">>], Req, State};
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetTerminalAccount' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
get_request_spec('GetTerminalAccount') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_response_spec('GetTerminalAccount', 200) ->
    {'TerminalAccount', 'TerminalAccount'};

get_response_spec('GetTerminalAccount', 400) ->
    {'BadRequest', 'BadRequest'};

get_response_spec('GetTerminalAccount', 401) ->
    undefined;

get_response_spec('GetTerminalAccount', 404) ->
    undefined;
